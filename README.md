...
```bash
curl -fsSL https://raw.githubusercontent.com/masooooood/Masoud-SSH-Tunnel/main/masoud-ssh-tunnel.sh \
| tr -d '\r' \
| tee /usr/local/bin/masoud-ssh >/dev/null \
&& chmod +x /usr/local/bin/masoud-ssh \
&& /usr/local/bin/masoud-ssh
```
